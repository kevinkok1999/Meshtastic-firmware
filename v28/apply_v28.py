...
""" + ws[pos:]
        ...
        ws = ws.replace(
            "var QCMDS=",
            "function v28Create(){modal('Create private channel','<div class=sec>SECURE PRIVATE CHANNEL</div><p>Choose a name. V28 creates the key and short join code automatically.</p><input id=v28name maxlength=31 placeholder=Channel autocomplete=off><button id=v28go>Create securely</button>');var b=E('v28go');if(b)b.onclick=function(){var n=(E('v28name').value||'').trim();closeOv();wsSend('@vc '+(n||'Private'))}}"
